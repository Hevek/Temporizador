...
namespace Temporizador
{
    public partial class Form1 : Form
    {
        ...
        {
            if (timerSegundos.Enabled == true)
            {
                if (hora.Text != "00")
                {
                    notifyIcon1.Text = $"Temporizador {hora.Text}:{minuto.Text}:{segundo.Text}";
                }
                else
                {
                    notifyIcon1.Text = $"Temporizador {minuto.Text}:{segundo.Text}";
                }
            }
            else
            {
                notifyIcon1.Text = "Temporizador";
            }

            if (player.playState == WMPLib.WMPPlayState.wmppsPlaying)
            {
                if (player.settings.mute == true || player.settings.volume == 0)
                {
                    notifyIcon1.Text += " (tocando mutado)";
                }
                else
                {
                    notifyIcon1.Text += " (tocando)";
                }
            }
            else
            {
         ...
         *  
         *  Próximos Objetivos:
         *      - Fazer o programa rodar em segundo plano || checked;
         *      - Colocar as funções CheckLabels(); e CheckEmptyString(); em uma classe, e utilizá-las como métodos(?).
         *      
         *  Últimas modificações feitas em: 25/01/2022 12:36*/
    }
}
